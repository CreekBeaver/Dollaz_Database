...
def get_dropdowns():
	...
	dropdowns = dict()	
	dropdowns['lease'] = []
	for n in db.execute_query(db_connection=db_connection, query = "SELECT * FROM lease;").fetchall():
		dropdowns['lease'].append([n['lease_id']])

	dropdowns['employee'] = []
	for n in db.execute_query(db_connection=db_connection, query = "SELECT * FROM employee;").fetchall():
		dropdowns['employee'].append([n['employee_id']])
	return dropdowns
...
def aircraft_assignment():
    ...
    if request.method == "GET":
        ...
        return render_template('aircraft_assignment.j2', data=results, dropdowns=dropdowns)
    if request.method == "POST":
        if 'add_row' in request.form.keys():
